...
def write_idx(areas):
    ...
    hcount = int(areas['data']['hcount'])
    mcount = int(areas['data']['mcount'])
    lcount = int(areas['data']['lcount'])
    ...
    df['mcount'] = [mcount]
    df['lcount'] = [lcount]
    type_list = {'public_date': sqlalchemy.Date(), 'public_time': sqlalchemy.Integer(), 'hcount': sqlalchemy.Integer(),
                 'mcount': sqlalchemy.Integer(), 'lcount': sqlalchemy.Integer()}
...
with open('./data/risk_data' + ' ' + public_time[0] + ' ' + public_time[1] + '.log', 'w', encoding='utf-8') as f:
    ...
    print("%s发布数据写入文件,其中高风险地区%s个，中风险地区%s个" % (
    public_time[0] + public_time[1], high_count, middle_count))
